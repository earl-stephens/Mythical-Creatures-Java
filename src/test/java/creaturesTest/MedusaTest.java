...
class MedusaTest {
	...
	@Test
	void testFourthVictimIsStonedMakesFirstUnstoned() {
		Person victim1 = new Person("Perseus");
		Person victim2 = new Person("Mercury");
		Person victim3 = new Person("Apollo");
		Person victim4 = new Person("Hades");
		...
		medusa.stare(victim4);
		assertEquals(3, medusa.statuesGetCount());	
		assertEquals("Mercury", medusa.getVictim(0).getName());
		assertEquals("Apollo", medusa.getVictim(1).getName());
		assertEquals("Hades", medusa.getVictim(2).getName());
		assertFalse(victim1.isStoned());
	}
}
